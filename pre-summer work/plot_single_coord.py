...
def plot_lon(file, sp1=None, sp2=None, sp3=None, scaled_sp=None):
    ...
    fig2, ax = plt.subplots(figsize=(6, 5))
    if sp1 is not None:
        ax.plot(np.log10(co2_lon), np.unique(lon), color='green', label='CO2')
    if scaled_sp is not None:
        ax.plot(np.log10(scaled_sp_lon), np.unique(lon), color='black', label='Scaled Species')
    if sp2 is not None:
        ax.plot(np.log10(nox_lon), np.unique(lon), color='red', label='NOx')
    if sp3 is not None:
        ax.plot(np.log10(h2o_lon), np.unique(lon), color='blue', label='H2O')
...
def plot_lat(file, sp1=None, sp2=None, sp3=None, scaled_sp=None):
    ...
    fig2, ax = plt.subplots(figsize=(6, 5))
    if sp1 is not None:
        ax.plot(np.log10(co2_lat), np.unique(lat), color='green', label='CO2')
    if scaled_sp is not None:
        ax.plot(scaled_sp_lat / 1e9, np.unique(lat), color='black', label='Scaled Species')
    if sp2 is not None:
        ax.plot(np.log10(nox_lat), np.unique(lat), color='red', label='NOx')
    if sp3 is not None:
        ax.plot(np.log10(h2o_lat), np.unique(lat), color='blue', label='H2O')
