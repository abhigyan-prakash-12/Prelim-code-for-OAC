...
def csv_to_matrix(csv_file):
    ...
    for row in matrix[1:]:
        co2 = float(row[1]) * 1e9         # CO2 emissions
        distance = float(row[6])          # Distance in km
        nox = float(row[2]) * 1e9         # NOx emissions
        h2o = float(row[3]) * 1e9         # H2O emissions
        soot = float(row[4]) * 1e9        # Soot emissions
        sulfur = float(row[5]) * 1e9      # Sulfur emissions
